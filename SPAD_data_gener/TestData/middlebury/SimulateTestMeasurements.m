...
for ss = 1:length(scenes)
    ...
    for zz = 1:size(simulation_params_T,1) 
        ...
        if LOW_RES
            ...
            img =depth;
            [h,w] = size(depth); %获取行和列，即原图的高度和宽度
 
            scale_w = 512; %根据输入获得缩放后的新宽度
            scale_h = 512; %根据输入获得缩放后的新高度
            ...
            intensity = output_img;

            depth_hr = depth;
            intensity_hr = intensity;

            depth = imresize(depth, 1/4, 'bicubic');        
            intensity = imresize(intensity, 1/4, 'bicubic');
            depth = max(depth, 0);
...
%         ambient_ppp = (mean_background_photons) .* intensity ./ mean(intensity(:));
        bp_1=ones(length(depth),length(depth));
        ambient_ppp = (mean_background_photons)*bp_1 ;
        %%
