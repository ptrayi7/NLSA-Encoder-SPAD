...
class NonLocalSparseAttention(nn.Module):
    def __init__(self, channels=64, k_size=1, reduction=4, chunk_size=144, res_scale=1, n_hashes=4):
        super(NonLocalSparseAttention, self).__init__()
        self.chunk_size = chunk_size
        self.n_hashes = n_hashes
        self.reduction = reduction
        self.res_scale = res_scale
        self.conv_match = model_block.BasicBlock(channels, channels // reduction, k_size, bn=False, act=None)
        self.conv_assembly = model_block.BasicBlock(channels, channels, 1, bn=False, act=None)
    ...
    def forward(self, inputs):
        ...
        x_embed = self.conv_match(inputs).view(N, -1, H * W).contiguous().permute(0, 2, 1)
        y_embed = self.conv_assembly(inputs).view(N, -1, H * W).contiguous().permute(0, 2, 1)
        L, C = x_embed.shape[-2:]
        ...
        x_embed_sorted = model_block.batched_index_select(x_embed, mod_indices)  # [N,n_hashes*H*W,C]
        y_embed_sorted = model_block.batched_index_select(y_embed, mod_indices)  # [N,n_hashes*H*W,C]
        ...
        ret = model_block.batched_index_select(ret, undo_sort)  # [N, n_hashes*H*W,C]
        ...
        ret = ret.permute(0, 2, 1).view(N, -1, H, W).contiguous() * self.res_scale + inputs
        return ret
